...
import {
    getPortMode,
    makeLabel,
} from 'lib/uniforms-surfnet/src/SubscriptionField';
    ...
    products: [
        {
            product_id: 'P1',
            tag: 'A',
            fixed_inputs: [{ name: 'port_speed', value: '1000' }],
        } as Product,
        {
            product_id: 'P2',
            tag: 'A',
            fixed_inputs: [{ name: 'port_speed', value: '10000' }],
        } as Product,
...
describe('<SubscriptionField>', () => {
    test('<SubscriptionField> - renders an input', async () => {
        ...
        const { element, getSubscriptions } =
            withSubscriptions(elementWithContext);
    ...
    test('<SubscriptionField> - renders a select with correct disabled state', async () => {
        const elementWithContext = withApplicationContext(
            <SubscriptionField name="x" disabled />,
            APP_CONTEXT,
        );
        const { element, getSubscriptions } =
            withSubscriptions(elementWithContext);
    ...
    test('<SubscriptionField> - renders a select with correct options (tags filtered)', async () => {
        const elementWithContext = withApplicationContext(
            <SubscriptionField name="x" tags={['A']} />,
            APP_CONTEXT,
        );
        const { element, getSubscriptions } =
            withSubscriptions(elementWithContext);
    ...
    test('<SubscriptionField> - renders a select with correct options (status filtered)', async () => {
        const { element, getSubscriptions } = withSubscriptions(
            <SubscriptionField name="x" statuses={['active']} />,
        );
        getSubscriptions.mockReturnValue([
            {
                subscription_id: 'a',
    ...
    test('<SubscriptionField> - renders a select with correct options (organisationId)', async () => {
        const { element, getSubscriptions } = withSubscriptions(
            <SubscriptionField name="x" organisationId="c2" />,
        );
        getSubscriptions.mockReturnValue([
            {
                subscription_id: 'a',
    ...
    test('<SubscriptionField> - renders a select with correct options (organisationKey not filled in yet)', async () => {
        const { element, getSubscriptions } = withSubscriptions(
            <SubscriptionField name="x" organisationKey="key" />,
        );
        getSubscriptions.mockReturnValue([
            {
                subscription_id: 'a',
        ...
        const wrapper = mount(
            element,
            createContext({ x: { type: String } }, { model: {} }),
        );
        await waitForComponentToPaint(wrapper);
    ...
    test('<SubscriptionField> - renders a select with correct options (organisationKey)', async () => {
        ...
        const wrapper = mount(
            element,
            createContext({ x: { type: String } }, { model: { key: 'c2' } }),
        );
        await waitForComponentToPaint(wrapper);
    ...
    test('<SubscriptionField> - renders a select with correct placeholder', async () => {
        const elementWithContext = withApplicationContext(
            <SubscriptionField name="x" placeholder="" />,
            APP_CONTEXT,
        );
        const { element, getSubscriptions } =
            withSubscriptions(elementWithContext);
        ...
        expect(wrapper.find(ReactSelect)).toHaveLength(1);
        expect(wrapper.find(ReactSelect).prop('placeholder')).toBe(
    ...
    test('<SubscriptionField> - renders a select with correct value (model)', async () => {
        const { element, getSubscriptions } = withSubscriptions(
            <SubscriptionField name="x" value="b" />,
        );
        getSubscriptions.mockReturnValue([
            {
                subscription_id: 'b',
        ...
        const wrapper = mount(
            element,
            createContext({ x: { type: String } }, { model: { x: 'b' } }),
        );
        await waitForComponentToPaint(wrapper);

        expect(wrapper.find(ReactSelect)).toHaveLength(1);
        expect(wrapper.find(ReactSelect).prop('value')).toStrictEqual({
            label: 'dec',
            value: 'b',
        });
    });
    ...
    test('<SubscriptionField> - renders a label', async () => {
        const elementWithContext = withApplicationContext(
            <SubscriptionField name="x" label="y" />,
            APP_CONTEXT,
        );
        const { element, getSubscriptions } =
            withSubscriptions(elementWithContext);
    ...
    test('<SubscriptionField> - renders a sync button', async () => {
        ...
        const wrapper = mount(
            ...
            createContext(
                { x: { type: String } },
                undefined,
                'subscriptionfield-syncbutton',
            ),
        );
        await waitForComponentToPaint(wrapper);

        // EUIButtonIcon and the surrounding div will receive the class
        expect(wrapper.find('.reload-subscriptions-icon-button')).toHaveLength(
            2,
        );
        // No service port should be visible
        expect(
            wrapper.find('.show-service-port-modal-icon-button'),
        ).toHaveLength(0);
        expect(wrapper.render()).toMatchSnapshot();
    });
    ...
    test('<SubscriptionField> - renders a select with correct value (as uniqueItem list child)', async () => {
        ...
        expect(wrapper.find(ReactSelect).at(0).prop('value')).toStrictEqual({
            label: 'dec1',
            value: 'a',
        });
        expect(wrapper.find(ReactSelect).at(1).prop('value')).toStrictEqual(
            undefined,
        );
        expect(wrapper.find(ReactSelect).at(1).prop('options')).toStrictEqual([
            { label: 'dec2', value: 'b' },
        ]);
        expect(wrapper.render()).toMatchSnapshot();
    });
    ...
    test('makeLabel - Works for Node subscriptions', async () => {
        expect(makeLabel(makeSubscription('Node'), [], [])).toBe(
            'abcdefgh dec',
        );
    });

    test('makeLabel - Works for Sn8 Port subscriptions', async () => {
        expect(makeLabel(makeSubscription('SP', 'tagged'), [], [])).toBe(
            'abcdefgh TAGGED dec abc',
        );
        expect(makeLabel(makeSubscription('SP', 'untagged'), [], [])).toBe(
            'abcdefgh UNTAGGED dec abc',
        );
        expect(makeLabel(makeSubscription('MSC'), [], [])).toBe(
            'abcdefgh TAGGED dec abc',
        );
    });

    test('getPortMode - Works for Sn8  subscriptions', async () => {
        expect(getPortMode(makeSubscription('SP', 'tagged'), [])).toBe(
            'tagged',
        );
        expect(getPortMode(makeSubscription('AGGSP', 'tagged'), [])).toBe(
            'tagged',
        );
        expect(getPortMode(makeSubscription('SP', 'untagged'), [])).toBe(
            'untagged',
        );
        expect(getPortMode(makeSubscription('AGGSP', 'untagged'), [])).toBe(
            'untagged',
        );
