...
import { ProductBlockDefinition, ProductDefinition, ResourceTypeDefinition, WorkflowDefinition } from '@/types';
...
export const getDefaultTableConfig = <T>(storageKey: string) => {
  switch (storageKey) {
    ...
    case METADATA_RESOURCE_TYPES_TABLE_LOCAL_STORAGE_KEY: {
      const resourceTypeColumns: (keyof ResourceTypeDefinition)[] = ['resourceTypeId'];
    ...
    case METADATA_WORKFLOWS_TABLE_LOCAL_STORAGE_KEY: {
      const workflowColumns: (keyof WorkflowDefinition)[] = ['workflowId', 'createdAt'];
      return getTableConfig<T>(workflowColumns as (keyof T)[]);
    }
    case METADATA_TASKS_TABLE_LOCAL_STORAGE_KEY: {
      const taskColumns: (keyof WorkflowDefinition)[] = ['workflowId', 'createdAt'];
