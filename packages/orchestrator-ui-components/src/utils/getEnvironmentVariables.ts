...
export function getEnvironmentVariables<T>(envVars: (keyof T)[]): Record<keyof T, string> {
  ...
  if (missingEnvironmentVariables.length > 0) {
    console.warn(`Warning: Missing required environment variables: ${missingEnvironmentVariables.join(', ')}`);
  }
