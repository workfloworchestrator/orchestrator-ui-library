...
import { EuiHorizontalRule, EuiSideNav, EuiSpacer, htmlIdGenerator } from '@elastic/eui';
...
export const WfoSidebar: FC<WfoSidebarProps> = ({ overrideMenuItems }) => {
  ...
  const defaultMenuItems: EuiSideNavItemType<object>[] = [
    {
      ...
      href: PATH_START,
      renderItem: () => (
        <WfoMenuItemLink path={PATH_START} translationString="start" isSelected={router.pathname === PATH_START} />
      ),
    },
    {
    ...
    },
    {
      ...
      renderItem: () => (
        <WfoMenuItemLink path={PATH_METADATA_PRODUCTS} translationString="metadata" hasSubItems={true} />
      ),
      items: [
        {
        ...
        },
        {
          ...
          isSelected: router.pathname === PATH_METADATA_SCHEDULED_TASKS,
  ...
  const defaultMenuItemsFilteredByPolicy = defaultMenuItems.filter(({ href }) =>
    menuItemIsAllowed(href, urlPolicyMap, isAllowed),
  );
  ...
  const defaultMenu: EuiSideNavItemType<object>[] = [
    {
      renderItem: () => (
        <>
          <WfoIsAllowedToRender resource={PolicyResource.SUBSCRIPTION_CREATE}>
