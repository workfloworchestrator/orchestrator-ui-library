...
export const WfoPageHeader: FC<WfoPageHeaderProps> = ({ navigationHeight, getAppLogo, onColorModeSwitch }) => {
      ...
      <EuiHeaderSection>
        <EuiHeaderSectionItem>
          ...
          {orchestratorConfig.useThemeToggle && (
            <EuiButtonIcon
              aria-label={t(colorMode === ColorModes.LIGHT ? 'darkMode' : 'lightMode')}
              display="empty"
              iconType={colorMode === ColorModes.LIGHT ? 'moon' : 'sun'}
              css={{
                width: theme.base * 3,
                height: theme.base * 3,
                color: theme.colors.textGhost,
              }}
              title={t(colorMode === ColorModes.LIGHT ? 'darkMode' : 'lightMode')}
              onClick={() => {
                onColorModeSwitch(colorMode === ColorModes.LIGHT ? ColorModes.DARK : ColorModes.LIGHT);
              }}
            />
          )}
