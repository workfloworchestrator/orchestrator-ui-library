...
export const getTimelineStyles = ({ theme }: WfoThemeHelpers) => {
  ...
  const getStepLineStyle = (timelinePosition: TimelinePosition, isFirstStep: boolean, isLastStep: boolean) => {
    ...
    return css([
      !isFirstStep && {
        '::before': {
          ...getHorizontalLineStyle(getBeforeLineColor(timelinePosition)),
        },
      },
      !isLastStep && {
        '::after': {
          ...getHorizontalLineStyle(getAfterLineColor(timelinePosition)),
        },
      },
    ]);
  };
  ...
  const getStepOuterCircleStyle = (hasContent: boolean) =>
    css({
      ...
      height: hasContent ? stepWithValueOuterDiameter : emptyStepOuterDiameter,
      width: hasContent ? stepWithValueOuterDiameter : emptyStepOuterDiameter,
    });

  const getStepInnerCircleStyle = (stepStatus: StepStatus, hasContent: boolean) => {
