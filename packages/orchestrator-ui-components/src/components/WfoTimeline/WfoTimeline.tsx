...
export const WfoTimeline: FC<WfoTimelineProps> = ({ timelineItems, indexOfCurrentStep = 0, onStepClick }) => {
  const { timelinePanelStyle } = useWithOrchestratorTheme(getTimelineStyles);

  const mapTimelineItemToStep = (timelineItem: TimelineItem, index: number, allTimelineItems: TimelineItem[]) => {
    ...
    return (
      <WfoTimelineStep
        ...
        timelinePosition={getTimelinePosition(index, indexOfCurrentStep)}
  ...
  return <div css={[timelinePanelStyle, useEuiScrollBar()]}>{timelineItems.map(mapTimelineItemToStep)}</div>;
};
