...
export const WfoTreeNode: FC<WfoTreeNodeProps> = ({ item, hasChildren, level }) => {
  const { theme } = useOrchestratorTheme();
  const { expandIconContainerStyle, treeContainerStyle } = useWithOrchestratorTheme(getStyles);
  const { expandedIds, collapseNode, expandNode, selectedIds } = React.useContext(TreeContext) as TreeContextType;
