...
export const getStyles = ({ theme, toSecondaryColor }: WfoThemeHelpers) => {
  ...
  const treeItemStyle = (isOutsideCurrentSubscription: boolean) => {
    return {
      ...
      border: isOutsideCurrentSubscription ? `1px dashed ${theme.colors.borderBaseSubdued}` : 'none',
    };
  };
  ...
  const selectedTreeItemStyle = (isOutsideCurrentSubscription: boolean) => {
    return {
      ...
      backgroundColor:
        isOutsideCurrentSubscription ? theme.colors.borderBaseSubdued : toSecondaryColor(theme.colors.primary),
