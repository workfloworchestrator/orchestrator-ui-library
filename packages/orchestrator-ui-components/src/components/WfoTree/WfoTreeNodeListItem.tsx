...
import { EuiButtonIcon, EuiFlexGroup, EuiFlexItem, EuiText, EuiTextTruncate } from '@elastic/eui';
...
export const WfoTreeNodeListItem = ({ item, selected }: WfoTreeNodeListItemProps) => {
  const t = useTranslations('common');
  const { toggleSelectedId } = useContext(TreeContext) as TreeContextType;
  const { theme } = useOrchestratorTheme();
  const { selectedTreeItemStyle, treeItemStyle } = useWithOrchestratorTheme(getStyles);
