...
export const WfoMultiCheckboxField: PydanticFormControlledElement = ({ pydanticFormField, onChange, value }) => {
  ...
  useEffect(() => {
    ...
    const getInitialMap = () => {
      const initialIdMap: idMap = (value || []).reduce((idMap: idMap, id: string) => {
        idMap[id] = true;
        return idMap;
      }, {} as idMap);
      return initialIdMap;
  ...
  const handleCheckboxChange = (optionId: string) => {
    ...
    const selectedIds = Object.keys(newCheckboxIdToSelectedMap).filter((key) => newCheckboxIdToSelectedMap[key]);
