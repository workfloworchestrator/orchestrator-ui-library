...
export const WfoAvailabilityCheck: FC<WfoAvailabilityCheckProps> = ({ featureType, availability, children }) => {
  if (!availability.isLoading && !availability.isAvailable) {
    return <WfoBackendUnavailable featureType={featureType} onRetry={() => window.location.reload()} />;
  }
