...
import { EuiButtonIcon, EuiFlexGroup, EuiFlexItem, EuiFormRow, EuiPanel } from '@elastic/eui';
...
import { createOptionsFromPaths, getPathSelectionOptions, isFullPathSelected, shouldHideValueInput } from './utils';
...
export const ConditionRow: FC<ConditionRowProps> = ({ condition, entityType, onChange, onRemove }) => {
  ...
  const selectedPathInfo: PathInfo | null = (() => {
    ...
    if (condition.path.includes('.')) {
      ...
      const fieldMatch = paths.find(({ path }) => path === condition.path);
      if (fieldMatch) return fieldMatch;
    }
  ...
  const hideValueInput = shouldHideValueInput(selectedPathInfo, !!condition.condition.op);
  const fullPathSelected = isFullPathSelected(condition.path, selectedPathInfo);
  ...
  const renderPathOption = (
    ...
    <WfoRenderPathOption option={option} searchValue={searchValue} contentClassName={contentClassName} paths={paths} />
  );
  ...
  return (
    <EuiPanel paddingSize="m" color="subdued">
      <EuiFlexGroup direction="column" gutterSize="m">
        <EuiFlexItem>
          <EuiFormRow label={t('fieldLabel')} error={error} isInvalid={!!error}>
            <EuiFlexGroup gutterSize="s" alignItems="center">
              ...
              {condition.path && selectedPathInfo?.ui_types && selectedPathInfo.ui_types.length > 0 && (
                <EuiFlexItem grow={false}>
                  <EuiFlexGroup gutterSize="xs" alignItems="center" responsive={false}>
                    {selectedPathInfo.ui_types.map((type, index) => (
                      <EuiFlexItem key={index} grow={false}>
                        <WfoBadge color={getTypeColor(type, theme)} textColor={theme.colors.textInk} size="s">
                          {type}
                        </WfoBadge>
                      </EuiFlexItem>
                    ))}
