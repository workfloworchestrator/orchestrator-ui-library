...
export const updateQueryString = (queryString: string, fieldName: string, value: string) => {
  ...
  // field:value
  const fieldRegex = new RegExp(String.raw`(${fieldName}):(".*?"|\(.*?\)|[^()|!:<>*"\s]+\*?)`, 'i');
  ...
  return queryString.replace(fieldRegex, `${existingFieldName}:(${updatedValue})`);
};
...
const toQueryValue = (value: string) => (hasSpaces(value) ? `"${value}"` : value);
