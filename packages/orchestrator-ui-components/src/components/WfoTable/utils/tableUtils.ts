...
export const determinePageIndex = (pageIndex: number, pageSize: number) => Math.floor(pageIndex / pageSize);
...
export const getDataSortHandler =
  <Type>(setDataDisplayParam: DataDisplayReturnValues<Type>['setDataDisplayParam']) =>
...
export const getQueryStringHandler =
  ...
  (queryString: string) => {
    ...
    const query = queryString && !hasSpecialCharacterOrSpace(queryString) ? `${queryString}*` : queryString;
...
export const getPageCount = (pagination: Pagination) =>
  ...
  Math.max(Math.ceil(pagination.totalItemCount / pagination.pageSize), pagination.pageIndex + 1);
