...
export const getTableConfigFromLocalStorage = <T>(key: string): StoredTableConfig<T> | undefined => {
...
export const setTableConfigToLocalStorage = <T>(key: string, updatedTableConfig: StoredTableConfig<T>) => {
  if (typeof window !== 'undefined' && window.localStorage) {
