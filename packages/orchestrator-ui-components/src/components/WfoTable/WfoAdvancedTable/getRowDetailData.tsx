...
import { ColumnType, WfoTableControlColumnConfigItem } from '@/components/WfoTable/WfoTable';
...
export const getRowDetailData = <T extends object>(
  ...
  const tableColumnConfigEntries: [
    string,
    WfoTableControlColumnConfigItem<T> | WfoAdvancedTableDataColumnConfigItem<T, keyof T>,
  ...
  return dataColumnEntries.map(([key, value]) => {
    ...
    return {
      ...
      value: (renderDetails && renderDetails(dataValue, selectedDataForDetailModal))
        ?? (renderData && renderData(dataValue, selectedDataForDetailModal)) ?? <>{dataValue}</>,
      textToCopy:
