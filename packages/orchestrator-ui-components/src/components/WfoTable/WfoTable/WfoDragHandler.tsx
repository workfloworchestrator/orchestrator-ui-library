...
export const WfoDragHandler: FC<WfoDragHandlerProps> = ({ headerRowRef, fieldName, onUpdateColumWidth }) => {
  ...
  const onStop: DraggableEventHandler = (_, data) => {
    if (headerRowRef.current && isDragging) {
      ...
      onUpdateColumWidth(fieldName, newWidth > MINIMUM_COLUMN_WIDTH ? newWidth : MINIMUM_COLUMN_WIDTH);
      resetPosition();
      setIsDragging(false);
    }
  };
  ...
  const thElement =
    ...
    && (headerRowRef.current.querySelector(`th[data-field-name="${fieldName}"]`) as HTMLTableCellElement);
