    ...
    <>
      {data.map((row, index) => (
        <Fragment key={`table-data-row-${index}`}>
              ...
              if (columnConfig.columnType === ColumnType.CONTROL) {
                return (
                  <td
                    ...
                    colSpan={columnConfig.numberOfColumnsToSpan ?? 1}
                    key={key}
                    css={[
                  ...
                  >
                    <div css={cellContentStyle}>{columnConfig.renderControl(row)}</div>
                  </td>
                );
              }
              ...
              return (
                ...
                >
                  <div css={cellContentStyle}>
                    <WfoDataCell customTooltip={columnConfig.renderTooltip?.(result, row)}>
                      {columnConfig.renderData?.(result, row) ?? result?.toString()}
                    </WfoDataCell>
                  </div>
                </td>
              );
          ...
          <WfoExpandedRow rowExpandingConfiguration={rowExpandingConfiguration} rowData={row} />
        </Fragment>
      ))}
    </>
  );
