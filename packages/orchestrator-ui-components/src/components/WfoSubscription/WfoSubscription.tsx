...
import { WfoSubscriptionStatusBadge, WfoSubscriptionSyncStatusBadge, WfoTitleWithWebsocketBadge } from '@/components';
...
export const WfoSubscription = ({ subscriptionId }: WfoSubscriptionProps) => {
  const [activeTab, setActiveTab] = useQueryParam(
    'activeTab',
    withDefault(StringParam, SubscriptionDetailTab.SERVICE_CONFIGURATION_TAB),
  );

  const selectedTab = ((): SubscriptionDetailTab => {
    return (
      subscriptionDetailTabs.find(({ id }) => id === activeTab)?.id || SubscriptionDetailTab.SERVICE_CONFIGURATION_TAB
    );
  })();
    ...
    <>
      {(isError && <WfoError />)
        ...
        || (subscriptionDetail && subscriptionDetail.subscriptionId && (
          <>
            <WfoContentHeader
              title={<WfoTitleWithWebsocketBadge title={subscriptionDetail.description} />}
              subtitle={
                <>
                  <WfoSubscriptionStatusBadge status={subscriptionDetail.status} />
                  <WfoSubscriptionSyncStatusBadge insync={subscriptionDetail.insync} />
                </>
              }
            >
            ...
            {selectedTab === SubscriptionDetailTab.SERVICE_CONFIGURATION_TAB && (
              <WfoSubscriptionDetailTree
                productBlockInstances={subscriptionDetail.productBlockInstances}
            ...
            {selectedTab === SubscriptionDetailTab.PROCESSES_TAB && data && subscriptionDetail.processes?.page && (
              <WfoProcessesTimeline subscriptionDetailProcesses={subscriptionDetail.processes?.page} />
            )}
            {selectedTab === SubscriptionDetailTab.RELATED_SUBSCRIPTIONS_TAB && data && (
              <WfoRelatedSubscriptions subscriptionId={subscriptionDetail.subscriptionId} />
            )}
          </>
