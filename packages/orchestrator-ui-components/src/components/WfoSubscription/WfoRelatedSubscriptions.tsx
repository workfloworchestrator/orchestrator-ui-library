...
import { ColumnType, Pagination, WfoTable, WfoTableColumnConfig } from '@/components/WfoTable/WfoTable';
import { mapSortableAndFilterableValuesToTableColumnConfig } from '@/components/WfoTable/WfoTable/utils';
import { useDataDisplayParams, useOrchestratorTheme } from '@/hooks';
import { WfoSearchStrikethrough } from '@/icons';
import { RelatedSubscriptionVariables, useGetRelatedSubscriptionsQuery } from '@/rtk';
...
export type RelatedSubscriptionListItem = Pick<
  SubscriptionListItem,
  'subscriptionId' | 'description' | 'status' | 'insync' | 'customerFullname' | 'tag' | 'startDate'
>;
...
export const WfoRelatedSubscriptions = ({
  ...
  const [hideTerminatedSubscriptions, setHideTerminatedSubscriptions] = useState<boolean>(true);
  ...
  const { dataDisplayParams, setDataDisplayParam } = useDataDisplayParams<RelatedSubscriptionListItem>({
    sortBy: {
      field: 'startDate',
  ...
  const graphqlQueryVariables: RelatedSubscriptionVariables = {
    ...
    terminatedSubscriptionFilter:
      hideTerminatedSubscriptions ?
        graphQlRelatedSubscriptionsTerminatedSubscriptionsFilterMapper(terminatedSubscriptionsFilter)
      : undefined,
  };

  const { data, isFetching, isLoading } = useGetRelatedSubscriptionsQuery(graphqlQueryVariables);

  const relatedSubscriptions = mapRelatedSubscriptionsResponseToRelatedSubscriptionsListItems(data);
  ...
  const tableColumns: WfoTableColumnConfig<RelatedSubscriptionListItem> = {
    ...
    description: {
      ...
      renderData: (value, record) => (
        <Link target="_blank" href={`${subscriptionPath}/${record.subscriptionId}`}>
          {value}
        </Link>
      ),
    },
  ...
  return (
    <>
      ...
      {(relatedSubscriptions?.length > 0
        ...
        <WfoNoResults text={t('noRelatedSubscriptions')} icon={<WfoSearchStrikethrough color={theme.colors.link} />} />
      )}
    </>
  );
};
