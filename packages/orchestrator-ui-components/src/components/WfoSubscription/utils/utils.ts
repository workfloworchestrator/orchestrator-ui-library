...
export const getFieldFromProductBlockInstanceValues = (
  ...
  const nameValue = instanceValues.find((instanceValue) => instanceValue.field === field);
  return nameValue?.value ?? '';
};

export const getProductBlockTitle = (instanceValues: FieldValue[]): string | number | boolean => {
  const title = getFieldFromProductBlockInstanceValues(instanceValues, 'title');
...
export const getLastUncompletedProcess = (
  ...
  return (uncompletedProcesses && uncompletedProcesses.length) > 0 ? uncompletedProcesses[0] : undefined;
};
...
export const sortProcessesByDate = (processList: SubscriptionDetailProcess[], sortOrder: SortOrder) => {
