...
export const mapRelatedSubscriptionsResponseToRelatedSubscriptionsListItems = (
  ...
  input?.relatedSubscriptions.map(({ subscriptionId, description, status, insync, customer, product, startDate }) => ({
    subscriptionId,
    description,
    status,
    insync,
...
const relatedSubscriptionsFieldMapper = (field: keyof RelatedSubscriptionListItem): keyof RelatedSubscription => {
  switch (field) {
