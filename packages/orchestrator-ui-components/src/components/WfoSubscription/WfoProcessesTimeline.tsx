...
const WfoProcessCard = ({ subscriptionDetailProcess }: WfoProcessCardProps) => {
  ...
  const { tableStyle, labelCellStyle, cellGroupStyle, borderStyle, rowStyle } =
  ...
  const rows = [
    {
      label: t('id'),
      content: (
        <Link href={`${processUrl}/${subscriptionDetailProcess.processId}`}>{subscriptionDetailProcess.processId}</Link>
      ),
    },
    {
      label: t('status'),
      content: (
        <div>
          <WfoProcessStatusBadge processStatus={subscriptionDetailProcess.lastStatus} />
        </div>
      ),
    },
    {
      label: t('startedAt'),
      content: parseDateToLocaleDateTimeString(parseDate(subscriptionDetailProcess.startedAt)),
    },
    {
      label: t('startedBy'),
...
const WfoRenderSubscriptionProcess = ({ subscriptionDetailProcess }: WfoRenderSubscriptionProcess) => {
...
export const WfoProcessesTimeline = ({ subscriptionDetailProcesses }: WfoProcessesTimelineProps) => {
