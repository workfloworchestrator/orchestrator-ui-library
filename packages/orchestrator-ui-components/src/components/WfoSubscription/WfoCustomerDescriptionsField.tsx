...
type CustomerDescriptionWithName = CustomerDescriptions & Partial<Pick<Customer, 'fullname' | 'shortcode'>>;
...
export const WfoCustomerDescriptionsField: FC<WfoCustomerDescriptionsFieldProps> = ({
  customerDescriptions,
  subscriptionCustomerId,
  subscriptionId,
}) => {
  const { customerDescriptionsCustomerNameStyle, customerDescriptionsFormStyle } =
  ...
  const customerIds = customerDescriptions.map((customerDescription) => customerDescription.customerId);
  ...
  const customerDescriptionsWithName: CustomerDescriptionWithName[] = customerDescriptions.map(
  ...
  return (
    <div css={customerDescriptionsFormStyle}>
      {(currentCustomerSubscriptionDescription && customerDescriptionEditForm(currentCustomerSubscriptionDescription))
        || customerDescriptionCreateForm()}
      {customerDescriptionsWithName
        .filter(({ customerId }) => customerId !== subscriptionCustomerId)
        .map((customerDescriptionWithName) => customerDescriptionEditForm(customerDescriptionWithName))}
    </div>
  );
};
