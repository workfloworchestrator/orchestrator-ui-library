...
import { getFieldFromProductBlockInstanceValues, getProductBlockTitle } from '../utils';
      ...
      >
        <EuiFlexGroup>
          <EuiFlexItem grow={false}>
            <div css={isOutsideCurrentSubscription ? iconOutsideCurrentSubscriptionStyle : iconStyle}>
          ...
          <EuiFlexItem>
            ...
            <EuiText size="s">
              {getFieldFromProductBlockInstanceValues(productBlock.productBlockInstanceValues, 'name')}
            </EuiText>
          </EuiFlexItem>
          <EuiFlexItem grow={false}>
            <EuiButtonEmpty
            ...
            <tbody>
              ...
              {showDetails && (
                <>
                  <tr key={-2} css={rowStyle}>
                    ...
                    <td>{productBlock.subscriptionInstanceId}</td>
                  </tr>
                  {!isOutsideCurrentSubscription && (
              ...
              {showProductBlockValues
                && productBlock.productBlockInstanceValues
                  .filter((productBlockInstanceValue) => !HIDDEN_KEYS.includes(productBlockInstanceValue.field))
                  .sort(({ field: fieldNameA }, { field: fieldNameB }) => {
                  ...
                  .map((productBlockInstanceValue, index) => {
                    if (productBlockInstanceValue && (!isEmpty(productBlockInstanceValue.value) || showDetails)) {
                      return (
