      ...
      scrollToStep: async (stepId: string) => {
        ...
        try {
          await new Promise((resolve, reject) => {
            const foundStepListItem = stepListItems.find((value) => value.step.stepId === stepId);

            if (!foundStepListItem) {
              return reject(undefined);
            }

            return resolve(onTriggerExpandStepListItem(foundStepListItem));
          });

          const targetRect = stepReferences.current.get(stepId)?.getBoundingClientRect();
          ...
          console.error('Error scrolling to step with stepId ', stepId);
        }
      },
    }));

    const getReferenceCallbackForStepId = (stepId: string) => (node: HTMLDivElement | null) => {
      if (node) {
