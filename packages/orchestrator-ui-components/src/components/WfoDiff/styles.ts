...
export const getWfoDiffStyles = ({ theme, toSecondaryColor, isDarkModeActive }: WfoThemeHelpers) => {
  ...
  const insertCodeColor =
    isDarkModeActive ? shade(insertGutterColor, SHADE_FACTOR) : tint(insertGutterColor, TINT_FACTOR);

  const deleteColor = toSecondaryColor(theme.colors.danger);
  const deleteSecondaryColor = isDarkModeActive ? shade(deleteColor, SHADE_FACTOR) : tint(deleteColor, TINT_FACTOR);
