...
import { DEFAULT_PAGE_SIZE, SUBSCRIPTIONS_TABLE_LOCAL_STORAGE_KEY } from '@/components';
...
export const WfoSubscriptionsListPage = () => {
  ...
  const getStoredTableConfig = useStoredTableConfig<SubscriptionListItem>(SUBSCRIPTIONS_TABLE_LOCAL_STORAGE_KEY);
  ...
  const { dataDisplayParams, setDataDisplayParam } = useDataDisplayParams<SubscriptionListItem>({
  ...
  const selectedTab = ((): WfoSubscriptionListTab => {
    return subscriptionListTabs.find(({ id }) => id === activeTab)?.id || WfoSubscriptionListTab.ACTIVE;
  })();

  const handleChangeSubscriptionsTab = (updatedSubscriptionsTab: WfoSubscriptionListTab) => {
