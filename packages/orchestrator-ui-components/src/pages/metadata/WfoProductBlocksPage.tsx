...
import { useGetProductBlocksQuery, useLazyGetProductBlocksQuery, useUpdateProductBlockMutation } from '@/rtk';
import type { ProductBlocksResponse } from '@/rtk';
import { mapRtkErrorToWfoError } from '@/rtk/utils';
import { BadgeType, GraphqlQueryVariables, ProductBlockDefinition, SortOrder } from '@/types';
import {
  csvDownloadHandler,
...
type ProductBlockDefinitionExportItem = Omit<ProductBlockDefinition, 'resourceTypes' | 'dependsOn'> & {
...
export const WfoProductBlocksPage = () => {
  ...
  const [tableDefaults, setTableDefaults] = useState<StoredTableConfig<ProductBlockDefinition>>();
  ...
  const { dataDisplayParams, setDataDisplayParam } = useDataDisplayParams<ProductBlockDefinition>({
  ...
  const tableColumns: WfoAdvancedTableColumnConfig<ProductBlockDefinition> = {
    ...
    name: {
      ...
      label: t('name'),
      width: '300px',
      renderData: (name) => <WfoProductBlockBadge badgeType={BadgeType.PRODUCT_BLOCK}>{name}</WfoProductBlockBadge>,
    },
    tag: {
      columnType: ColumnType.DATA,
      label: t('tag'),
    ...
    dependsOn: {
      ...
      renderData: (dependsOn) => (
        <>
          {dependsOn.map(({ name }, index) => (
            <WfoProductBlockBadge
              key={index}
              link={getQueryUrl(PATH_METADATA_PRODUCT_BLOCKS, `name:"${name}"`)}
      ...
      renderTooltip: (productBlocks) => {
        return productBlocks.map((productBlock) => <p key={productBlock.name}>- {productBlock.name}</p>);
      },
    },
    resourceTypes: {
      columnType: ColumnType.DATA,
      ...
      renderData: (resourceTypes) => (
        <>
          {resourceTypes.map(({ resourceType }, index) => (
            <WfoProductBlockBadge
              key={index}
              link={getQueryUrl(PATH_METADATA_RESOURCE_TYPES, `resourceType:"${resourceType}"`)}
      ...
      renderTooltip: (resourceTypes) => {
        return resourceTypes.map((resourceType) => (
          <p key={resourceType.resourceType}>- {resourceType.resourceType}</p>
        ));
      },
    },
    createdAt: {
  ...
  const { data, isFetching, error } = useGetProductBlocksQuery(graphqlQueryVariables);
  const [getProductBlocksTrigger, { isFetching: isFetchingCsv }] = useLazyGetProductBlocksQuery();
  const getProductBlocksForExport = () =>
    getProductBlocksTrigger(getQueryVariablesForExport(graphqlQueryVariables)).unwrap();
  ...
  const mapToExportItems = (productBlocksResponse: ProductBlocksResponse): ProductBlockDefinitionExportItem[] => {
    const { productBlocks } = productBlocksResponse;
    return productBlocks.map((productBlock) => ({
      ...productBlock,
      resourceTypes: getConcatenatedResult(productBlock.resourceTypes, ['resourceType', 'description']),
  ...
  return (
    <WfoMetadataPageLayout>
      <WfoAdvancedTable
        ...
        onUpdateQueryString={getQueryStringHandler<ProductBlockDefinition>(setDataDisplayParam)}
