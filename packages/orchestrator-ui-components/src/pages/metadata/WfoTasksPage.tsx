...
import { ColumnType, Pagination } from '@/components/WfoTable/WfoTable/WfoTable';
...
import { TasksResponse, useGetTasksQuery, useLazyGetTasksQuery, useUpdateWorkflowMutation } from '@/rtk';
...
import { graphQlTaskListMapper, mapTaskDefinitionToTaskListItem } from './taskListObjectMapper';

export type TaskListItem = Pick<TaskDefinition, 'workflowId' | 'name' | 'description' | 'target' | 'createdAt'> & {
  productTags: string[];
  scheduleFrequency: ScheduleFrequency[];
};

export type TaskListExportItem = Omit<TaskListItem, 'productTags' | 'scheduleFrequency'> & {
  productTags: string;
};
...
const ScheduleTaskPopoverMenu = ({ workflowId }: ScheduleTaskPopoverMenuProps) => {
  ...
  return (
    <WfoPopover
      ...
      isLoading={false}
      PopoverContent={() => <SetScheduleButton workflowId={workflowId} closePopover={() => setIsPopoverOpen(false)} />}
      button={button}
...
const SetScheduleButton = ({ workflowId, closePopover }: { workflowId: string; closePopover: () => void }) => {
  ...
  return (
    <EuiContextMenuItem
      ...
      onClick={() => {
        closePopover();
        router.push(`${PATH_METADATA_ADD_SCHEDULE_TASK_FORM}/?workflowId=${workflowId}`);
      }}
    >
      {t('addSchedule')}
    </EuiContextMenuItem>
...
export const WfoTasksPage = () => {
  ...
  const [tableDefaults, setTableDefaults] = useState<StoredTableConfig<TaskListItem>>();
  const getStoredTableConfig = useStoredTableConfig<TaskListItem>(METADATA_TASKS_TABLE_LOCAL_STORAGE_KEY);
  ...
  const { dataDisplayParams, setDataDisplayParam } = useDataDisplayParams<TaskListItem>({
  ...
  const tableColumns: WfoAdvancedTableColumnConfig<TaskListItem> = {
    ...
    name: {
      columnType: ColumnType.DATA,
      label: t('name'),
      renderData: (name) => <WfoProductBlockBadge badgeType={BadgeType.TASK}>{name}</WfoProductBlockBadge>,
      width: '300px',
    },
    description: {
      columnType: ColumnType.DATA,
    ...
    productTags: {
      ...
      renderData: (productTags) => (
        <>
          {productTags?.filter(onlyUnique).map((productTag, index) => (
            <WfoProductBlockBadge
              key={index}
              link={getQueryUrl(PATH_METADATA_PRODUCTS, `tag:"${productTag}"`)}
      ...
      renderTooltip: (productTags) => {
        return productTags
          ...
          .map((productTag) => <p key={productTag}>- {productTag}</p>);
      },
    },
    scheduleFrequency: {
      columnType: ColumnType.DATA,
      label: t('scheduled'),
      renderData: (_, taskListItem) => <WfoScheduledTasksBadgesContainer workflowId={taskListItem.workflowId} />,
      width: '80px',
      isSortable: false,
    },
    createdAt: {
    ...
    addSchedule: {
      ...
      width: '80px',
      renderControl: (taskListItem) => <ScheduleTaskPopoverMenu workflowId={taskListItem.workflowId} />,
    },
  };
  ...
  const { data, isFetching, error } = useGetTasksQuery(taskListQueryVariables);

  const [getTasksTrigger, { isFetching: isFetchingCsv }] = useLazyGetTasksQuery();

  const getTasksForExport = () => getTasksTrigger(getQueryVariablesForExport(taskListQueryVariables)).unwrap();
  ...
  const mapToExportItems = (tasksResponse: TasksResponse): TaskListExportItem[] => {
    const { tasks } = tasksResponse;

    return tasks.map(({ workflowId, name, target, description, createdAt, products }) => {
      return {
        workflowId,
        name,
        target,
  ...
  return (
    <WfoMetadataPageLayout>
      <WfoAdvancedTable
        ...
        tableColumnConfig={mapSortableAndFilterableValuesToTableColumnConfig(tableColumns, sortFields, filterFields)}
        dataSorting={[dataSorting]}
        defaultHiddenColumns={tableDefaults?.hiddenColumns}
        onUpdateDataSorting={getDataSortHandler<TaskListItem>(setDataDisplayParam)}
        onUpdateQueryString={getQueryStringHandler<TaskListItem>(setDataDisplayParam)}
