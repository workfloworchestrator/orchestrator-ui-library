...
import { WorkflowsResponse, useGetWorkflowsQuery, useLazyGetWorkflowsQuery, useUpdateWorkflowMutation } from '@/rtk';
...
import { graphQlWorkflowListMapper, mapWorkflowDefinitionToWorkflowListItem } from './workflowListObjectMapper';
...
export const WfoWorkflowsPage = () => {
  ...
  const [tableDefaults, setTableDefaults] = useState<StoredTableConfig<WorkflowListItem>>();

  const getStoredTableConfig = useStoredTableConfig<WorkflowListItem>(METADATA_WORKFLOWS_TABLE_LOCAL_STORAGE_KEY);
  ...
  const { dataDisplayParams, setDataDisplayParam } = useDataDisplayParams<WorkflowListItem>({
  ...
  const tableColumns: WfoAdvancedTableColumnConfig<WorkflowListItem> = {
    ...
    name: {
      columnType: ColumnType.DATA,
      label: t('name'),
      renderData: (name) => <WfoProductBlockBadge badgeType={BadgeType.WORKFLOW}>{name}</WfoProductBlockBadge>,
      width: '350px',
    },
    description: {
      columnType: ColumnType.DATA,
  ...
  const { data, isFetching, error } = useGetWorkflowsQuery(workflowListQueryVariables);

  const [getWorkflowsTrigger, { isFetching: isFetchingCsv }] = useLazyGetWorkflowsQuery();

  const getWorkflowsForExport = () =>
    getWorkflowsTrigger(getQueryVariablesForExport(workflowListQueryVariables)).unwrap();
  ...
  const mapToExportItems = (workflowsResponse: WorkflowsResponse): WorkflowListExportItem[] => {
    const { workflows } = workflowsResponse;
    return workflows.map(({ workflowId, name, target, description, createdAt, products }) => ({
      workflowId,
      name,
      target,
      description,
  ...
  return (
    <WfoMetadataPageLayout>
      <WfoAdvancedTable
        data={data ? mapWorkflowDefinitionToWorkflowListItem(data.workflows) : []}
        tableColumnConfig={mapSortableAndFilterableValuesToTableColumnConfig(tableColumns, sortFields, filterFields)}
        dataSorting={[dataSorting]}
        defaultHiddenColumns={tableDefaults?.hiddenColumns}
        onUpdateDataSorting={getDataSortHandler<WorkflowListItem>(setDataDisplayParam)}
        onUpdateQueryString={getQueryStringHandler<WorkflowListItem>(setDataDisplayParam)}
