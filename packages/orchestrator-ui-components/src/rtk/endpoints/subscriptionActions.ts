...
const subscriptionActionsApi = orchestratorApi.injectEndpoints({
  endpoints: (build) => ({
    getSubscriptionActions: build.query<SubscriptionActions, { subscriptionId: string }>({
      query: ({ subscriptionId }) => `${SUBSCRIPTION_ACTIONS_ENDPOINT}/${subscriptionId}`,
      extraOptions: {
...
export const { useGetSubscriptionActionsQuery, useLazyGetSubscriptionActionsQuery } = subscriptionActionsApi;
