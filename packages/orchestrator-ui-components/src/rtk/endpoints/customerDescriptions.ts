...
const customerDescriptionsApi = orchestratorApi.injectEndpoints({
  endpoints: (build) => ({
    setCustomerDescription: build.mutation<
      null,
      Pick<CustomerDescriptions, 'description' | 'customerId' | 'subscriptionId'>
    >({
...
export const { useSetCustomerDescriptionMutation, useUpdateCustomerDescriptionMutation } = customerDescriptionsApi;
