...
function isSerializedError(error: SerializedError | undefined): error is SerializedError {
  if (error) {
    return (
      error
      && typeof error === 'object'
      && ('name' in error || 'message' in error || 'stack' in error || 'code' in error)
    );
  }
  return false;
}
...
export const getWebSocket = async (url: string) => {
  ...
  if (session?.accessToken) {
    ...
    return new WebSocket(url, ['base64.bearer.token', session?.accessToken]);
  } else {
