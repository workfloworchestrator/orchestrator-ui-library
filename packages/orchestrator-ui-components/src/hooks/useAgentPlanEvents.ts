...
export function useAgentPlanEvents(
    agentId: string = 'query_agent',
): PlanExecutionState {
    const { agent } = useAgent({ agentId });
    const [executionState, setExecutionState] =
        useState<PlanExecutionState>(initialState);
    ...
    useEffect(() => {
        ...
        const subscriber: AgentSubscriber = {
                    ...
                    setExecutionState((prev) => {
                        ...
                        if (existingIndex >= 0) {
                            steps[existingIndex] = {
                                ...
                                status: 'active',
                                reasoning:
                                    reasoning ?? steps[existingIndex].reasoning,
                            };
                        } else {
                            steps.push({
                                step_name: stepName,
            ...
            onToolCallStartEvent: ({ event }) => {
                setExecutionState((prev) => {
                    const currentStep = prev.steps.find(
                        (s) => s.status === 'active',
                    );
