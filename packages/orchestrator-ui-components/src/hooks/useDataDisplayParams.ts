...
export const useDataDisplayParams = <Type>(
  ...
  const [dataDisplayParams, setDataDisplayParams] = useQueryParams({
    ...
    sortBy: withDefault(ObjectParam, defaultParams.sortBy ? defaultParams.sortBy : {}),
    queryString: withDefault(StringParam, defaultParams.queryString ? defaultParams.queryString : ''),
  });
