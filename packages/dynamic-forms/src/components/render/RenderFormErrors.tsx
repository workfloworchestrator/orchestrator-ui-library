...
export default function RenderFormErrors() {
	...
	return (
		<div className="mb-4 mt-4 error-box d-flex justify-content-between align-items-center">
				...
				{!!otherErrors.length && (
					<>
						<div className="d-flex align-items-center">
							Er {multiMistakes ? "zijn" : "is"} {otherErrors?.length} rubriek
							...
							<ul className="error-list mb-2">
								{otherErrors.map((error) => (
									<li key={JSON.stringify(error)}>
										<strong>{error.loc.map(getFieldLabel).join(", ")}</strong>:{" "}
										{error.msg}
									</li>
								))}
							</ul>
			...
			<IconWaarschuwing style={{ opacity: 0.4 }} className="mr-3" size={40} />
		</div>
	)
}
